...
class ChatComponent:
    ...
    def _render_welcome_message(self) -> None:
        ...
        <div class="welcome-message">
            <h4>🚀 Welcome to the BigTool-Powered Mathematical Agent!</h4>
            <p>I use <strong>LangGraph BigTool</strong> for intelligent tool selection and professional mathematical reasoning:</p>
            <ul>
                <li><strong>🧮 Integral Calculus:</strong> Definite and indefinite integrals with symbolic and numerical methods</li>
                <li><strong>� Function Analysis:</strong> Derivatives, critical points, limits, and behavior analysis</li>
                <li><strong>� Interactive Visualization:</strong> Function plots with area highlighting and mathematical graphics</li>
                <li><strong>� Smart Tool Selection:</strong> BigTool automatically chooses the best tools for your problem</li>
                <li><strong>� Natural Language Processing:</strong> Just describe your mathematical problem naturally</li>
            </ul>
            <p><strong>🎯 Example:</strong> "Calculate the integral of x² from 0 to 3 and show me the area under the curve"</p>
            <p><em>The agent will automatically extract the expression (x²), bounds (0 to 3), and generate both calculation and visualization!</em></p>
        </div>
    ...
    def _process_user_message(self, message: str) -> None:
        """Process user message using refactored BigTool controller."""
        try:
            ...
            # Use refactored controller with BigTool
            result = controller.process_message(message)
            
            if result.get("success", True):  # Default to success for backward compatibility
                # Extract response and metadata from BigTool result
                response_content = result.get("response", "No response received")
                tools_used = result.get("tools_used", [])
                processing_time = result.get("processing_time", 0)
                
                # Check for plots generated by tools BEFORE adding message
                from app.tools.langchain_tools import get_generated_plots
                generated_plots = get_generated_plots()
                ...
                self.state_manager.add_message({
                    ...
                    "visualizations": generated_plots,  # Add plots directly here
                    "metadata": {
                        "execution_time": processing_time,
                        "agent_type": result.get("metadata", {}).get("agent_type", "bigtool_direct"),
                        "tool_calls": result.get("metadata", {}).get("tool_calls", [])
                    }
                })
                
            else:
                # Handle error from BigTool
